                        ...
                        const std::vector<unsigned char>& vchPlaintext = std::vector<unsigned char>())
{
    ...
    // Only the recovered plaintext is meaningful when both implementations
    // report success. On a padding failure (random ciphertext) both return
    // false and leave undefined bytes in the output buffers, whose exact
    // contents differ between OpenSSL versions (older OpenSSL zeroes the final
    // block, newer ones leave the raw decrypt), so comparing them is not valid.
    if (result1)
